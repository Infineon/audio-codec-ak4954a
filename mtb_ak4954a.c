...
static cyhal_i2c_t* _ak4954a_i2c_ptr;
...
cy_rslt_t mtb_ak4954a_init(cyhal_i2c_t* i2c_inst)
{
    ...
    _ak4954a_i2c_ptr = i2c_inst;

    // From the AK4954A spec:
    // Upon power-up, the AK4954A must be reset by bringing the PDN pin low. This reset is released
    // when a dummy command is input after the PDN pin is high. This ensures that all internal
    // registers reset to their initial value. This reset is released when the dummy command
    // (Actually, the rising edge of 16th SCL) is input after PDN pin is high. Dummy command is
    // executed by writing all 0's to the register address 00H. It is recommended to set the PDN pin
    // low before power up the AK4954A.
    //
    // We don't want to call mtb_ak4954a_write_byte() here because we will hang on the CY_ASSERT()
    // check.  The second call to cyhal_i2c_master_write() is needed to clean up the error that
    // occurs with the first one.
    {
        uint8_t buf[] = { AK4954A_REG_PWR_MGMT1, 0 };
        cyhal_i2c_master_write(_ak4954a_i2c_ptr, AK4954A_I2C_ADDR, buf, sizeof(buf), 0, true);
        cyhal_i2c_master_write(_ak4954a_i2c_ptr, AK4954A_I2C_ADDR, buf, sizeof(buf), 0, true);
    }
...
void mtb_ak4954a_write_byte(mtb_ak4954a_reg_t reg, uint8_t data)
{
    ...
    rslt = cyhal_i2c_master_write(_ak4954a_i2c_ptr, AK4954A_I2C_ADDR, buf, sizeof(buf), 0, true);
...
void mtb_ak4954a_write_data_stream(mtb_ak4954a_reg_t reg, uint8_t* pData, uint8_t numBytes)
{
    ...
    rslt = cyhal_i2c_master_write(_ak4954a_i2c_ptr, AK4954A_I2C_ADDR, buf, numBytes + 1, 0, true);
    CY_UNUSED_PARAMETER(rslt); // CY_ASSERT only processes in DEBUG, ignores for others
    CY_ASSERT(CY_RSLT_SUCCESS == rslt);
}
...
uint8_t mtb_ak4954a_read_byte(mtb_ak4954a_reg_t reg)
{
    ...
    rslt =
        cyhal_i2c_master_write(_ak4954a_i2c_ptr, AK4954A_I2C_ADDR, (uint8_t*)(&reg), 1, 0, false);
    CY_UNUSED_PARAMETER(rslt); // CY_ASSERT only processes in DEBUG, ignores for others
    CY_ASSERT(CY_RSLT_SUCCESS == rslt);

    rslt = cyhal_i2c_master_read(_ak4954a_i2c_ptr, AK4954A_I2C_ADDR, &data, 1, 0, true);
    CY_UNUSED_PARAMETER(rslt); // CY_ASSERT only processes in DEBUG, ignores for others
    CY_ASSERT(CY_RSLT_SUCCESS == rslt);
...
uint16_t mtb_ak4954a_read_word(mtb_ak4954a_reg_t reg)
{
    ...
    rslt = cyhal_i2c_master_read(_ak4954a_i2c_ptr, AK4954A_I2C_ADDR, (uint8_t*)&data, 2, 0, true);
    CY_UNUSED_PARAMETER(rslt); // CY_ASSERT only processes in DEBUG, ignores for others
    CY_ASSERT(CY_RSLT_SUCCESS == rslt);
...
void mtb_ak4954a_read_data_stream(mtb_ak4954a_reg_t reg, uint8_t* pData, uint8_t numBytes)
{
    ...
    rslt = cyhal_i2c_master_read(_ak4954a_i2c_ptr, AK4954A_I2C_ADDR, pData, numBytes, 0, true);
    CY_UNUSED_PARAMETER(rslt); // CY_ASSERT only processes in DEBUG, ignores for others
    CY_ASSERT(CY_RSLT_SUCCESS == rslt);
}
...
//
// Activates the codec - This function is called in conjunction with ak4954A_deactivate API after
// successful configuration update of the codec.
//--------------------------------------------------------------------------------------------------
