...
cy_rslt_t mtb_ak4954a_init(cyhal_i2c_t *i2c_inst)
{
     ...
     * Upon power-up, the AK4954A must be reset by bringing the PDN pin = “L”. This reset is
     * released when a dummy command is input after the PDN pin = “H”. This ensures that all
     * internal registers reset to their initial value. This reset is released when the dummy
     * command (Actually, the rising edge of 16th SCL) is input after PDN pin = “H”. Dummy
     * command is executed by writing all “0” to the register address 00H. It is recommended
     * to set the PDN pin = “L” before power up the AK4954A.
     *
